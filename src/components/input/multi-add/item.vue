  ...
  setup(props, ctx) {
    return () => (
      <li class="max-w-full flex items-center py-1 pr-0 pl-3 rounded-3.5 relative overflow-hidden">
        <span class="relative top-px whitespace-nowrap overflow-hidden text-base overflow-ellipsis text-white z-20">
          {props.item}
        </span>
        <button
        ...
        >
          <div class="flex text-white text-opacity-70 group-hover:text-dark group-hover:text-opacity-100 group-focus:text-dark group-focus:text-opacity-100">
            <BCMSIcon
              src="/close"
              class="fill-current w-6 m-auto transition-all duration-200 flex-shrink-0"
            />
          </div>
        </button>
        <div class="absolute top-0 left-0 w-full h-full block bg-green z-10 transition-colors duration-300 peer-hover:bg-red peer-focus:bg-red" />
      </li>
    );
  },
});
